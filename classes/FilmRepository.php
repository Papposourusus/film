...
class FilmRepository {
    ...
    public function update(Film $film) {
        $stmt = $this->conn->prepare("UPDATE films SET title = :title, description = :description, duration = :duration, image = :image WHERE id = :id");
        $stmt->bindParam(':title', $film->title);
        $stmt->bindParam(':description', $film->description);
        $stmt->bindParam(':duration', $film->duration, PDO::PARAM_INT);
        $stmt->bindParam(':image', $film->image);
        $stmt->bindParam(':id', $film->id, PDO::PARAM_INT);
        $stmt->execute();
    }
